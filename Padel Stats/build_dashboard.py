...
def build_dashboard_html(data: dict) -> str:
    ...
    # NOTE: Keep this as a plain triple-quoted string (not an f-string) to avoid JS brace escaping.
    html = """<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<body>
  ...
  function renderHeatmap(containerId, playersOrdered, matrix, maxVal, modeLabel, restsMap) {
    ...
    if (!playersOrdered.length) { el.innerHTML = '<div class="muted">No regular players loaded.</div>'; return; }
  ...
  function renderAudit() {
    const el = byId('audit');
    if (DATA.sessions.length === 0) { el.innerHTML = '<div class="muted">No sessions found in this folder.</div>'; return; }
    let html = '';
  ...
  function refresh() {
    ...
    if (regularPlayers.length) byId('regularNote').textContent = `Regulars loaded: ${regularPlayers.length} (guests ignored).`;
    else byId('regularNote').textContent = "No regular list found (put app.py or regular_players.txt next to build_dashboard.py). Falling back to all names in data.";

    const usedPlayers = [];
    if (regularPlayers.length) usedPlayers.push(...regularPlayers);
    else {
      const set = new Set();
      included.forEach(fn => {
